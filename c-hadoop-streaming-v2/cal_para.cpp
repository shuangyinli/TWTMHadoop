...
void run(char* model_root, char* prefix, char* infofile, int num_learn_pi) {
    ...
    for (int i = 0; i < model->num_labels; i++) model->pi[i] /= num_learn_pi;
...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        printf("usage: <model_root_dir> <prefix> <info file> <num_learn_pi>\n");
        return -1;
    }
    run(argv[1], argv[2], argv[3], atoi(argv[4]));
    return 0;
}
