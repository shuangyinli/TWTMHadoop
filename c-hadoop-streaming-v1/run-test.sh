...
if [ $# -ne 6 ]
then
    echo "usage: <input file> <hdfs model.tar.gz> <out dir> <hdfs input dir> <hdfs output dir> <num reducer>"
    exit -1
fi
...
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/contrib/streaming/*streaming*.jar \
                ...
                -file "test-mapper" \
                -jobconf mapred.job.name="[SSLDA-inference-hadoopv2-lessnetworkcost]" \
